...
//        setupPermissions();
    ...
    public void delete(){
        ...
        if(row>=0) {
            try {
                String grainCode = (String) view.getDtm().getValueAt(row, 0);
                int confirm = JOptionPane.showConfirmDialog(view, "确定要删除这条记录吗？", "确认删除", JOptionPane.YES_NO_OPTION);
                if (confirm==JOptionPane.YES_OPTION){
                List<Grain> grian2 = iGrainInfService.delete1(grainCode);
                DefaultTableModel model = view.getDtm();
                model.setRowCount(0);
                for (Grain grain : grian2) {
    ...
    public void loadGrainData() {
        try {
            ...
            for (WareHouse wareHouse : wareHouses) {
                if(wareHouse.getNotice().equals("")){wareHouse.setNotice("0");}
                model.addRow(new Object[]{
                        wareHouse.getId(),
